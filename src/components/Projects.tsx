function Project() {
  ...
  return (
    <section className="bg-surface-container-low py-32" id="work">
      <div className="max-w-7xl mx-auto px-8">
        <div className="flex flex-col md:flex-row justify-between items-end mb-20 gap-8">
          ...
          <div className="hidden md:block">
            <span className="text-xs font-bold tracking-[0.3em] uppercase opacity-30">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          {projects.map((p, i) => (
            <div className="bg-surface-container-lowest p-10 flex flex-col justify-between min-h-[500px] group cursor-pointer hover:bg-primary transition-colors duration-500">
              <div>
                <div className="flex justify-between items-start mb-12">
                  <span className="font-label text-xs font-bold tracking-widest opacity-50 group-hover:text-on-primary">
                    {"PROJECT_" + `0${i + 1}`}
                  </span>
                  <a
                    href={p.link}
                    className="material-symbols-outlined text-4xl group-hover:text-on-primary transition-transform group-hover:rotate-45"
                    target="_blank"
                ...
                <h3 className="text-3xl font-bold mb-6 group-hover:text-on-primary">{p.name}</h3>
                <p className="text-on-surface-variant leading-relaxed mb-8 group-hover:text-on-primary/80">
                  {p.description}
                </p>
              </div>
              <div className="flex flex-wrap gap-2">
                {p.tools.map((t) => (
                  <span className="border border-outline px-3 py-1 text-[0.65rem] uppercase font-bold group-hover:border-on-primary group-hover:text-on-primary">
                    {t}
                  </span>
                ))}
              </div>
